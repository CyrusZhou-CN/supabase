...
Text frames are always JSON encoded but differently from version 1.0.0, a JSON array is used where the order is necessarily:
...
See below the same example as the previous section using hexadecimal representation for the header and plain text for the rest with the following values:
...
Structure of the broadcast event that is received by all clients in a channel. The `payload` field contains the event name and data that was broadcasted.
...
This message is a streamlined version of the text frame broadcast event that also supports non-JSON payloads.
See below the same example as the previous section using hexadecimal representation for the header and plain text for the actual fields with the following values:
...
Server sent message with a change from a schema and table. This message is sent when a change occurs in the database that the client is subscribed to. The payload contains the details of the change, including the schema, table, event type, and the new and old records.
