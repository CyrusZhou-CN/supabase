...
If your email templates are not being updated, check [Auth Logs](https://supabase.com/dashboard/project/_/logs/auth-logs) for any errors when the email is sent. When there are errors, we will use a fallback template with valid syntax so it will not show any of your customizations.

Typical errors relate to invalid syntax involving variables. Refer to our [Email Templates docs](/docs/guides/auth/auth-email-templates#terminology) for the supported variables and syntax.
